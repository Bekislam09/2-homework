  ...
  function getWeather() {
    axios.get(`https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=metric&appid=${API_KEY}`)
      .then((response) => {
        ...
        const dailyForecasts = forecasts.filter(item => item.dt_txt.includes('12:00:00')).slice(0, 4);
  ...
  return (
    <div id="cont">
      <div id="card">
        ...
        <div id="forecast">
          {
          forecasts.length > 0 && forecasts.map((item, index) => (
            <div key={index}>
              <p id='date'>{item.dt_txt.split('')[0]}</p>
              <p>{item.main.temp}°</p>
              <p>{item.weather[0].main}</p>
            </div>
          ))
